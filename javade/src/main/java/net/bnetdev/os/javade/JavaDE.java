...
public class JavaDE extends Project
{
    ...
    public JavaDE() {
    	JavaDE.getLogger().setDebugMode(true);
    	this.initLocale();
		this.greet();
    }
	...
	public void greet() {
        ...
        ApplicationConstantModule m = (ApplicationConstantModule) constants.getController().getModule(ApplicationConstantModule.NAME);
        String title = m.getConstant("NAME")
                .concat(" v")
                .concat(m.getConstant("VERSION"))
                .concat(" - Developed By - ")
        ...
        JavaDE.getLogger().info("Running ".concat(StringUtils.capitalize(platform.getPlatform().toString().toLowerCase())).concat(" Kernel ").concat(platform.getVersion()));
	}
	...
	private void initConstants(AbstractLocaleModule alm) {
		...
		ApplicationConstantModule app = (ApplicationConstantModule) constant.getController().getModule(ApplicationConstantModule.NAME);
	...
	private void initNetwork(AbstractLocaleModule alm, ApplicationConstantModule acm) {
		...
		ExternalResolverModule erm = (ExternalResolverModule) network.getController().getModule(ExternalResolverModule.NAME); 
